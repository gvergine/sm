#!/bin/bash
VALGRIND_ERR=10
VALGRIND_SMI="valgrind --leak-check=full --show-leak-kinds=all -s --error-exitcode=$VALGRIND_ERR --trace-children=yes --errors-for-leak-kinds=all $1"
TESTS_DIR=$2
...
function expect_fail_result_but_ok_mem() {
    ...
    if [[ $RET -eq $VALGRIND_ERR ]]; then echo "Valgrind found errors"; exit 1; fi
}
